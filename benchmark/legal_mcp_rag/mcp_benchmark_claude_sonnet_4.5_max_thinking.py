...
class MCPBenchmark:
    def __init__(self, api_key: Optional[str] = None, mcp_servers: Optional[List[Dict]] = None):
        """
        MCP 벤치마크 초기화
...
answer: 3"""
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                # 스트리밍으로 응답 받기 (10분 이상 걸릴 수 있는 요청 지원)
                response_text = ""
                thinking_text = ""
                mcp_tool_uses = []
                input_tokens = 0
        ...
        with self.print_lock:
            ...
            query_input_cost = (result['tokens_input'] / 1_000_000) * 3
    ...
    def print_summary(self):
        ...
        # 비용 계산 (Claude Sonnet 4.5 가격)
        # Input: $3/MTok (≤200K), Output: $15/MTok (≤200K)
...
def main(limit: int = None, workers: int = 10):
    """메인 실행 함수"""
    ...
    TEMPERATURE = 1  # thinking 모드에서는 temperature=1 필수
    MAX_TOKENS = 64000
    THINKING_MODE = True  # extended thinking 활성화
    THINKING_BUDGET = 63999  # max budget_tokens
    MAX_WORKERS = workers
    ...
    mcp_servers = [
        {
            ...
            "tool_configuration": {
                "enabled": True,
                "allowed_tools": ["search_cases", "get_case_content", "search_statutes", "get_statute_content", "list_statute_articles"]
            },
        }
    ]

    # API 키 설정 (환경변수가 없으면 기본값 사용)
    api_key = os.environ.get("ANTHROPIC_API_KEY") or "your-anthropic-api-key"
    ...
    # 벤치마크 실행 - 항상 0부터 시작
    benchmark = MCPBenchmark(api_key=api_key, mcp_servers=mcp_servers)
    ...
    output_file = "result/mcp_benchmark_claude_sonnet_4.5_max_thinking_result.json"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MCP RAG Benchmark - Claude Sonnet 4.5 Max Thinking")
