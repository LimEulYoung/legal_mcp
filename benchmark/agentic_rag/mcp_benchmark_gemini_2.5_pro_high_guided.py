...
class MCPBenchmark:
    ...
    async def run_single_query(
        ...
        Returns:
            실행 결과 딕셔너리
        """
        # 프롬프트에 MCP 활용 지시 및 출력 형식 지시 추가
        formatted_prompt = f"""당신은 한국 법률 전문가입니다. 아래 법률 문제를 풀기 위해 반드시 MCP 도구를 활용하여 관련 법령과 판례를 검색하고 분석하세요.

## 문제 풀이 절차

1. 문제를 분석하여 관련 법령과 쟁점 파악
2. search_statutes로 관련 법령 검색 → get_statute_content로 조문 확인
3. search_cases로 관련 판례 검색 → get_case_content로 판례 내용 확인
4. 위 과정을 충분히 반복하면서 추가 검색
5. 충분한 법적 근거를 확보한 후 최종 답변 도출

※ 법령의 구조를 모르거나 조문 번호를 모를 때는 list_statute_articles로 해당 법령의 조문 목록을 먼저 확인하세요.

**중요: 정확한 답변을 위해 도구를 반복적으로 호출하세요. 최소 10회 이상의 도구 호출을 권장합니다.**
...
answer: 3"""
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                # 응답이 비어있거나 파싱 실패 또는 MCP 미사용시 재시도
...
async def main(limit: int = None, workers: int = 5):
    ...
    THINKING_BUDGET = 32768  # HIGH에 해당하는 thinking budget
    ...
    output_file = "result/mcp_benchmark_gemini_2.5_pro_high_guided_result.json"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MCP RAG Benchmark - Gemini 2.5 Pro High Guided")
