#!/usr/bin/env python3
"""
Ablation 실험: MCP 도구 조합별 성능 비교 (GPT)
...
사용 예시:
  python ablation_no_case_content_gpt.py --condition statute_only --model gpt-5.1 --effort high --workers 3
  python ablation_no_case_content_gpt.py --condition no_case_content --model gpt-5.1 --effort high --workers 3
"""
...
class GPTBenchmark:
    def __init__(self, model: str, effort: str, allowed_tools: List[str] = None,
                 api_key: Optional[str] = None, mcp_server_url: Optional[str] = None):
        self.model = model
        self.effort = effort
...
def main():
    parser = argparse.ArgumentParser(description="Ablation: MCP 도구 조합별 성능 비교 (GPT)")
    parser.add_argument("--condition", type=str, required=True,
                        choices=list(ABLATION_CONDITIONS.keys()),
                        help="ablation 조건: no_case_content / statute_only / full")
    ...
    benchmark = GPTBenchmark(model=model, effort=effort, allowed_tools=cond_info["allowed"], mcp_server_url=MCP_SERVER_URL)
