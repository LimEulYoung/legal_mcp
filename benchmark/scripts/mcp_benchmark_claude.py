#!/usr/bin/env python3
"""
Anthropic API 기반 MCP 벤치마크 스크립트
Anthropic SDK의 beta MCP client를 사용하여 법률 문제 풀이

사용 예시:
  # Claude Sonnet 4.5 + high effort
  python mcp_benchmark_claude.py --model claude-sonnet-4-5-20250929 --effort high --workers 3

  # Claude Sonnet 4.5 + max effort
  python mcp_benchmark_claude.py --model claude-sonnet-4-5-20250929 --effort max --workers 3

  # Claude Haiku 4.5 + medium effort, 2025년 문제
  python mcp_benchmark_claude.py --model claude-haiku-4-5-20251001 --effort medium --csv ../benchmark_2025.csv --workers 3
"""
...
answer: 3"""
        ...
        with self.print_lock:
            ...
            if result["status"] == "success":
                print(f"정답: {result['expected_answer']} | 추출된 답: {result['extracted_answer']} | 채점: {'O 정답' if result['is_correct'] else 'X 오답'}")
            else:
...
def main():
    parser = argparse.ArgumentParser(description="Claude API MCP 벤치마크")
    parser.add_argument("--model", type=str, default=DEFAULT_MODEL, help=f"Claude 모델명 (기본: {DEFAULT_MODEL})")
    parser.add_argument("--effort", type=str, default="high", choices=["none", "low", "medium", "high", "max"], help="thinking effort 레벨 (기본: high)")
    ...
    if args.output:
        ...
        # 모델명에서 파일명 생성 (예: claude-sonnet-4-5-20250929 → claude_sonnet_4_5_20250929)
    ...
    metadata = {
        "experiment_name": "MCP RAG Benchmark (Claude API)",
        "model": model,
    ...
    benchmark = ClaudeMCPBenchmark(model=model, effort=effort, mcp_server_url=MCP_SERVER_URL)
